...
describe('GithubOAuthGuard', () => {
  ...
  beforeEach(() => {
    ...
    superCanActivateSpy = jest.spyOn(Object.getPrototypeOf(GithubOAuthGuard.prototype), 'canActivate');
  });
  ...
  describe('handleRequest', () => {
    it('should return user if no error occurs and user is present', () => {
      const mockUser = { id: 1, email: 'github-user@example.com' };
    ...
    it('should throw the provided error if err is present', () => {
      const customError = new Error('GitHub verification failed');
